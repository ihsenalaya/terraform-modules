# Module Terraform — `azurerm_linux_web_app` (public/privé via Private Endpoint)

> **MAJ** : compatible dernières versions stables — **Terraform `>= 1.13.0`** et **AzureRM `~> 4.48`**.
>
> Règle simple : `public = true` ➜ Web App publique. `public = false` ➜ le module crée **Private Endpoint** + **association Private DNS Zone**.
